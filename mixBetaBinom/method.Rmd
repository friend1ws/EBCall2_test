...
```{r, echo=FALSE}
evalTimeBetaBinomEst(20, 50, 0, 5, 5);
```


### Discussion

* The results show that the computational times are not that slow, and the current naive approach is feasible for whole genome sequence data if using paralelizaion.
* But current algorithm is just directly using 'ConstOptim', and do not consider any characteristics specific to Beta-Binomial distribution.
* I believe we have much room for further improvements.
